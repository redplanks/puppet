class ocf_puppet::puppetserver {
  ...
  augeas { '/etc/default/puppetserver':
    ...
    changes => [
      "set JAVA_ARGS '\"-Xms1536m -Xmx1536m -XX:ReservedCodeCacheSize=512m\"'",
    ],
    require => Package['puppetserver'],
    notify  => Service['puppetserver'],
  }

  $docker_private_hosts = lookup('kubernetes::worker_nodes')

  # Allow Kubernetes workers to access docker secrets
